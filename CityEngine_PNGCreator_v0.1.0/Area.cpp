...
// boundary getters
short Area::GetMinBoundX()
{
    return minBoundX;
}

short Area::GetMaxBoundX()
{
    return maxBoundX;
}

short Area::GetMinBoundY()
{
    return minBoundY;
}

short Area::GetMaxBoundY()
{
    return maxBoundY;
}
...
/// <param name="g"></param>
/// <param name="b"></param>
Area::Area(short x1, short x2, short y1, short y2, double r, double g, double b)
{
    // store bounds
    minBoundX = x1;
    maxBoundX = x2;
...
bool Area::FillNeighbor(short x, short y, std::vector<bool>* fillMap, std::vector<bool>* checkedMap, short prob)
{
    // if filled, skip
...
/// <summary>
/// unused draw method (can't be displayed)
/// </summary>
void Area::Draw()
{
...
/// <summary>
/// saves to area type map
/// </summary>
/// <param name="areas">area type map</param>
void Area::Save(pngwriter* areas)
{
    // writes a medium blend 
    for (int i = 0; i < filled.size(); i += 2)
    {
        areas->filledsquare_blend(filled[i] - 20, filled[i + 1] - 20, filled[i] + 20, filled[i + 1] + 20, 0.01, red, green, blue);
    }
}